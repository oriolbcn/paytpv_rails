    ...
    def add_user(ds_merchant_pan, ds_merchant_expirydate, ds_merchant_cvv2, ds_original_ip)
      ...
      response = client.request "add_user" do
       soap.body do |xml|
         ...
         xml.DS_MERCHANT_MERCHANTSIGNATURE(Paytpv::Signatures.add_user_signature(@paytpv_client, ds_merchant_pan, ds_merchant_cvv2, @paytpv_terminal, @paytpv_password))
    ...
    def info_user(ds_iduser, ds_token_user, ds_original_ip)
      ...
      response = client.request "info_user" do
       soap.body do |xml|
         ...
         xml.DS_MERCHANT_MERCHANTSIGNATURE(Paytpv::Signatures.info_user_signature(@paytpv_client, ds_iduser, ds_token_user, @paytpv_terminal, @paytpv_password))
         xml.DS_ORIGINAL_IP(ds_original_ip)
       end
      end
    ...
    def execute_purchase(ds_iduser, ds_token_user, ds_merchant_amount, ds_merchant_order, ds_merchant_currency, ds_original_ip, ds_merchant_productdescription = "", ds_merchant_owner = "")
      ...
      response = client.request "execute_purchase" do
       soap.body do |xml|
         ...
         xml.DS_MERCHANT_MERCHANTSIGNATURE(Paytpv::Signatures.execute_purchase_signature(@paytpv_client + ds_iduser + ds_token_user +  @paytpv_terminal + ds_merchant_amount + ds_merchant_order + @paytpv_password))
         xml.DS_ORIGINAL_IP(ds_original_ip)
